            ...
            for device_name, device_info in self.controller.device_list.items():
                ...
                for idx in range(1, device_count + 1):
                    ...
                    for config_topic, payload in configs:
                        # topic|payload 형식으로 TCP 전송
                        message = f"{config_topic}|{json.dumps(payload)}"
                        self.controller.send_tcp(message.encode('utf-8'))

            self.logger.info("TCP Discovery 설정 완료")
            
        except Exception as e:
            self.logger.error(f"TCP Discovery 설정 중 오류 발생: {str(e)}")